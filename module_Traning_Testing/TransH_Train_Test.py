...
class Training_Testing:
    def __init__(self, entity_set, relation_set, triple_list, embedding_dim=50, lr=0.01, margin=1.0, norm=1, C=1.0,
                 epsilon=1e-5, valid_triple_list=None):
    ...
    def data_initialise(self):
        self.model = TransH.TransH(len(self.entities), len(self.relations), self.dimension, self.margin, self.C, self.epsilon,
                       self.norm)
    ...
    def training_run(self, epochs=300, batch_size=100, out_file_title=''):
        ...
        with codecs.open(
                out_file_title + "TransH_pytorch_entity_" + str(self.dimension) + "dim_batch" + str(batch_size),
                "w") as f1:
        ...
        with codecs.open(
                out_file_title + "TransH_pytorch_norm_relations_" + str(self.dimension) + "dim_batch" + str(batch_size),
                "w") as f2:
        ...
        with codecs.open(out_file_title + "TransH_pytorch_hyper_relations_" + str(self.dimension) + "dim_batch" + str(
                batch_size), "w") as f3:
